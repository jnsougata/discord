...
import (
	"encoding/json"
	"github.com/jnsougata/disgo/channel"
	"github.com/jnsougata/disgo/emoji"
	"github.com/jnsougata/disgo/member"
	"github.com/jnsougata/disgo/role"
)
...
type Guild struct {
	...
	Roles                       map[string]role.Role       `json:"x_roles"`
	...
	Members                     map[string]member.Member   `json:"x_members"`
	Channels                    map[string]channel.Channel `json:"x_channels"`
...
func Unmarshal(payload interface{}) *Guild {
	guild := &Guild{}
...
func (guild *Guild) UnmarshalMembers(objs []interface{}) {
	var members = map[string]member.Member{}
	for _, o := range objs {
		uo := member.Unmarshal(o)
		uo.GuildId = guild.Id
...
func (guild *Guild) UnmarshalRoles(objs []interface{}) {
	var roles = map[string]role.Role{}
	for _, o := range objs {
		uo := role.Unmarshal(o)
		uo.GuildId = guild.Id
		roles[uo.Id] = *uo
...
func (guild *Guild) UnmarshalChannels(objs []interface{}) {
	var chs = map[string]channel.Channel{}
	for _, o := range objs {
		uo := channel.Unmarshal(o)
		chs[uo.Id] = *uo
	}
	guild.Channels = chs
}
